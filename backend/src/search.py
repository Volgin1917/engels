...
class SemanticSearchService:
        ...
        try:
            with self.SessionLocal() as session:
                ...
                sql = text("""
                    ...
                    LIMIT :top_k
                """).compile(dialect=self.engine.dialect, compile_kwargs={"literal_binds": True})
                    ...
                    results.append(
                        {
                            ...
                            "similarity": 1.0
                            - float(row.distance) / 2.0,  # Convert to similarity score
                        }
                    )

                logger.info("Semantic search completed", results_count=len(results), top_k=top_k)
    ...
    def search_by_text(
        self, query_text: str, vectorizer, top_k: int = 5, source_id: int | None = None
    ) -> list[dict[str, Any]]:
        """
        ...
        return self.search_similar(query_embedding=embedding, top_k=top_k, source_id=source_id)
