...
from .models import Entity as EntityModel, Relation as RelationModel, Source
from .entity_extractor import Entity, Relation, EntityType, RelationType, get_entity_extractor
...
class GraphBuilder:
    ...
    async def process_chunk_entities(
        ...
        # Save to database
        saved_entities = await self._save_entities(
            unique_entities,
            source_id=source_id
        )
    ...
    async def _save_entities(
        ...
        for entity in entities:
            ...
            entity_hash = self._generate_entity_hash(entity.name, entity.entity_type)
            ...
            if existing_entity:
                ...
                existing_entity.extra_metadata = {
                    **existing_entity.extra_metadata,
                    **entity.metadata,
                    "mentions_count": existing_entity.extra_metadata.get("mentions_count", 0) + len(entity.mentions)
                }
        ...
        for relation in relations:
            ...
            if existing_relation:
                ...
                existing_relation.confidence_score = max(
                    existing_relation.confidence_score,
                    relation.confidence
                )
                ...
                new_relation = RelationModel(
                    ...
                    status="verified" if relation.confidence >= 0.8 and not relation.needs_review else "raw",
                )
    ...
    async def get_entities_needing_review(
        self, 
        limit: int = 100
    ) -> List[EntityModel]:
        ...
        stmt = select(EntityModel).where(
            EntityModel.needs_review == True
        ).limit(limit)
        
        result = await self.db.execute(stmt)
        return list(result.scalars().all())
    
    async def get_relations_needing_review(
        self, 
        limit: int = 100
    ) -> List[RelationModel]:
        ...
        stmt = select(RelationModel).where(
            RelationModel.status == "raw"
        ).limit(limit)
        
        result = await self.db.execute(stmt)
        return list(result.scalars().all())
    
    async def verify_entity(self, entity_id: int, verified: bool, comment: Optional[str] = None) -> bool:
    ...
    async def verify_relation(self, relation_id: int, verified: bool, comment: Optional[str] = None) -> bool:
    ...
    async def get_graph_statistics(self) -> Dict[str, Any]:
        ...
        predicate_stmt = select(
            RelationModel.predicate,
            func.count(RelationModel.id)
        ).group_by(RelationModel.predicate)
        ...
        review_stmt = select(func.count(EntityModel.id)).where(
            EntityModel.needs_review == True
        )
        ...
        # Count raw relations
        raw_stmt = select(func.count(RelationModel.id)).where(
            RelationModel.status == "raw"
        )
