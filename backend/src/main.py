...
async def semantic_search(
    ...
    try:
        ...
        results = search_service.search_by_text(
            query_text=q, vectorizer=vectorizer, top_k=top_k, source_id=source_id
        )
...
async def create_source(source: SourceCreate, background_tasks: BackgroundTasks):
    ...
    raise HTTPException(
        status_code=501, detail="Source creation not yet implemented. Use direct file upload."
    )
...
async def process_source(source_id: int, use_mcp: bool = False):
    ...
    try:
        ...
        return {"source_id": source_id, "task_id": task.id, "status": "queued"}
...
async def health_check():
    ...
    components = {"database": False, "ollama": False, "redis": False}
    ...
    try:
        ...
        return HealthResponse(
            status="unhealthy", components=components, timestamp=datetime.utcnow()
        )
