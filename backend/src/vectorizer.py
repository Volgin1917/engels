...
class OllamaClient:
    ...
    def __init__(self, host: str = None, model: str = None):
        ...
        logger.info("OllamaClient initialized", host=self.host, model=self.model)
    ...
    async def generate_embedding(self, text: str) -> list[float] | None:
        ...
        try:
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                ...
                if embedding:
                    logger.debug(
                        "Embedding generated", text_length=len(text), vector_dim=len(embedding)
                    )
                    return embedding
                else:
    ...
    async def generate_embeddings_batch(
        ...
        async def bounded_generate(text: str) -> list[float] | None:
    ...
    def generate_embedding_sync(self, text: str) -> list[float] | None:
        """
...
class VectorizationService:
    ...
    async def vectorize_chunks(self, chunks: list[tuple], source_id: int) -> list[dict[str, Any]]:
        """
        ...
        [idx for idx, _ in chunks]

        logger.info("Starting vectorization", source_id=source_id, chunk_count=len(chunks))
    ...
    def vectorize_chunks_sync(self, chunks: list[tuple], source_id: int) -> list[dict[str, Any]]:
        """
    ...
    async def verify_and_prepare_model(self) -> bool:
        ...
        if self.client.model not in models:
            logger.warning("Required model not found, attempting to pull", model=self.client.model)
